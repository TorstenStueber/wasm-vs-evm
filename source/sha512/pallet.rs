    ...
    *state = H;
        ...
        .skip(data_length as usize + 1)
    {
        *element = if new_length - 1 - position < 8 {
            (bit_length >> ((new_length - 1 - position) * 8)) as u8
        } else {
            0
        };
    }
...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn odd_product(origin: OriginFor<T>, n: u8, iterations: u32) -> DispatchResult {
            let result = super::iterated_hashes(n, iterations);

            // Emit an event.
            Self::deposit_event(Event::Result(result));
